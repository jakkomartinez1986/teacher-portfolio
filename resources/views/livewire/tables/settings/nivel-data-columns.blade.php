<thead>
    <tr class="text-xs border-b uppercase font-semibold tracking-wider">
        ...
        <th class="px-4 py-3"> Jornada Academica </th> 
        <th class="px-4 py-3">
            <a wire:click.prevent="sortBy('nivel_name')" href="#" class="hover:underline flex items-center gap-1">
                Nivel
                @include('components.app-components.sort-icon', ['field' => 'nivel_name'])
            </a>
        </th> 
...
<tbody class="text-sm ">
    ...
    <tr class="hover:bg-blue-100 dark:hover:bg-blue-900 transition-colors duration-200 border-b">
        ...
        <td class="px-4 py-3">{{ $item->nivel_name }}</td>
        <td class="px-4 py-3">
        ...
        <td class="px-4 py-3">
            <div class="flex items-center space-x-2">
                <flux:dropdown position="bottom" align="end" offset="-15">
                    ...
                    <flux:menu>
                        <flux:menu.item icon="eye" size="sm" href="{{route('settings.nivels.show', $item)}}">Ver</flux:menu.item>
                        @can('editar-'.$modelactive)
                            <flux:menu.item icon="pencil" size="sm" href="{{route('settings.nivels.edit', $item->id)}}" >Editar</flux:menu.item>
                        @endcan
