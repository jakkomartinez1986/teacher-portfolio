<thead>
    <tr class="text-xs border-b uppercase font-semibold tracking-wider">
        ...
        <th class="px-4 py-3">
            <a wire:click.prevent="sortBy('year_name')" href="#" class="hover:underline flex items-center gap-1">
                Año de Gestion 
                @include('components.app-components.sort-icon', ['field' => 'year_name'])
            </a>
        </th> 
...
<tbody class="text-sm ">
    ...
    @forelse($data as $item)
        <tr class="hover:bg-blue-100 dark:hover:bg-blue-900 transition-colors duration-200 border-b">
            ...
            <td class="px-4 py-3">{{ $item->year_name }}</td>
            ...
            <td class="px-4 py-3">
                <div class="flex items-center space-x-2">
                    <flux:dropdown position="bottom" align="end" offset="-15">
                        ...
                        <flux:menu>
                            <flux:menu.item icon="eye" size="sm" href="{{route('settings.years.show', $item)}}">Ver</flux:menu.item>
                            @can('editar-'.$modelactive)
                                <flux:menu.item icon="pencil" size="sm" href="{{route('settings.years.edit', $item->id)}}" >Editar</flux:menu.item>
                            @endcan
