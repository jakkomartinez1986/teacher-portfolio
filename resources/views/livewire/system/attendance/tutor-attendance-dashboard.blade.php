<div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
    ...
    <div class="flex flex-col gap-6 rounded-xl border border-neutral-200 bg-white p-6 dark:border-neutral-700 dark:bg-neutral-800">
        <h2 class="text-xl font-semibold text-neutral-800 dark:text-neutral-100">
            Tutor: Dashboard de Asistencias y Novedades del Grado
        </h2>
    ...
    @if(count($grades) > 0)
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="rounded-lg border border-neutral-200 bg-green-50 p-4 dark:border-neutral-700 dark:bg-green-900/30">
                <h3 class="text-sm font-medium text-green-800 dark:text-green-200">Tasa de Asistencia</h3>
                                ...
                                <div class="mb-3">
                                    ...
                                    {{-- @if($class['class']->teacher())
                                        <p class="text-sm text-gray-600 dark:text-gray-400">
                                            Docente: {{$class['class']->teacher->name }}
                                        </p>
                                    @endif --}}
                                    @if($class['classtopic'])
                                    ...
                                    @endif
                                    @if($class['teacher'])
                                        <p class="text-sm text-neutral-600 dark:text-neutral-300"><strong>Docente:</strong> {{ $class['teacher'] }}</p>
                                    @endif
                                                    ...
                                                    <td class="border border-neutral-200 px-3 py-2 text-center dark:border-neutral-700">
                                                        @if($att['status_text'] && $att['phone'])
                                                            @if(!$att['notification_sent_at'])
                                                              @php
                                                                $teacherTutor = Auth::user()->full_name ?? 'Docente Tutor';
                                                                $subject = $class['class'] ?? 'Asignatura no especificada';
                                                                $teacherName = $class['teacher'] ?? 'Docente Asignado';
                                                                $message = rawurlencode(
                                                                    ...
                                                                    "📌 *Acción requerida:*\n" .
                                                                    "Se solicita de manera obligatoria presentarse en la institución educativa con el docente de la hora durante el horario de atención a padres de familia o coordinar con el, a fin de:\n" .
                                                                    "1. Analizar las causas que han generado esta situación\n" .
                                                                    "2. Establecer compromisos y estrategias de mejora conjunta\n\n" .
                                                                    "📄 *Marco legal:*\n" .
                                                                    "De conformidad con el Art. 6, literal g) de la *Ley Orgánica de Educación Intercultural*, es deber del representante legal: *'Vigilar la asistencia, el progreso académico y la conducta de sus hijos, hijas o representados en el proceso educativo.'*\n\n" .
                                                                    "🔔 Su presencia y compromiso son fundamentales para garantizar el desarrollo académico integral del estudiante.\n\n" .
                                                                    "Atentamente,\n" .
                                                                    "*{$teacherTutor}*\n" .
                                                                    "Docente - Tutor"
                                                                );
